...
Device Activation UX ya existe en `/dashboard`. Usa el formulario `Activar identificador` con input `public_id`, placeholder `PID-XXXXXXXXXX`, boton `Activar identificador`, estado `Activando...` y exito `Identificador activado correctamente.`. El cliente frontend actual es `activateDevice(publicId, accessToken): Promise<Device>` en `apps/web/lib/devices.ts`, usa `buildApiUrl` y todavia debe actualizarse para enviar `claim_code`. El backend ya requiere `public_id + claim_code`, por lo que el dashboard actual puede quedar temporalmente incompatible hasta el sprint frontend.
