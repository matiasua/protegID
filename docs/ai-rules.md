...
El consentimiento publico es explicito, usa `PUBLIC_PROFILE_CONSENT_VERSION`, no se infiere desde `is_public` y no se expone publicamente. La fuente de verdad productiva para readiness/publicacion/acceso publico es `apps/api/app/services/emergency_profile_status.py`, que separa tres dominios: `ProfileReadiness` (solo `EmergencyProfile`), `PublicationEligibility` (agrega consentimiento) y `PublicAccessStatus` (agrega Device + ProtectedPerson, especifico de un device/public_id). `apps/api/app/services/profile_readiness.py` (motor legacy `calculate_profile_readiness(device, profile)` que mezclaba Device+EmergencyProfile) ya no tiene callers productivos; se mantiene solo por compatibilidad con posibles consumidores externos no migrados. `GET /api/devices/{device_id}/emergency-profile/readiness` (DEPRECATED, ver parrafo siguiente) y el schema `EmergencyProfileReadinessRead` requieren autenticacion, verifican ownership y no exponen valores medicos, `user_id` ni `device_id`.

Bloque 8.1 (deprecation legacy): los tres endpoints device-scoped `GET /api/devices/{device_id}/emergency-profile`, `PUT /api/devices/{device_id}/emergency-profile` y `GET /api/devices/{device_id}/emergency-profile/readiness` estan marcados `deprecated=True` en OpenAPI y responden header `Deprecation: true`, pero siguen 100% funcionales (deprecated != removed: no hay evidencia suficiente para descartar consumidores externos). Los equivalentes account-scoped vigentes son `GET /api/emergency-profile`, `PUT /api/emergency-profile` y `GET /api/emergency-profile/status`. `GET /api/devices/{device_id}/public-access-status` NO es legacy: es contrato productivo actual, no esta deprecated y sigue siendo consumido por el frontend.
